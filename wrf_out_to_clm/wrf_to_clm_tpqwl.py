...
for m in month:
    ...
    new_array = new_array.rename({'T2':'TBOT','Q2':'SHUM','PSFC':'PSRF','GLW':'FLDS'}) # rename to be consistent with CLM(FATES)

    # Create a new dataset
    ds = xr.Dataset({'TBOT':(['time','x','y'], new_array['TBOT']),
                      'SHUM': (['time','x','y'], new_array['SHUM']),
    ...
    ds['SHUM'].attrs = [('description','1-HOURLY MEAN GRID SCALE SPECIFIC HUMIDITY'), ('units','kg/kg')]
    ...
    ds.to_netcdf("/home/kmurenbeeld/scratch/wrf_to_clm/clmforc_WRF30d02_c2018_1kmx1km_" + str(year) + "_" + str(m) +  "_TPQWL.nc")
